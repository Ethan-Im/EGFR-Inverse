...
with T790M-targeted candidates showing preferential binding to the mutant receptor.
...
\subsection{Docking Validation}

AutoDock Vina docking of top-5 candidates confirmed binding to the EGFR
kinase pocket, with affinities ranging from $-7.6$ to $-8.3$ kcal/mol
(Table~\ref{tab:docking}).
T790M-targeted candidates showed preferential binding to the T790M receptor
(best: $-8.310$ kcal/mol) compared to wild-type candidates on the WT receptor
(best: $-8.124$ kcal/mol), consistent with mutant-selective design.

\begin{table}[h]
\centering
\caption{AutoDock Vina docking scores for top candidates.}
\label{tab:docking}
\begin{tabular}{lcc}
\toprule
...
candidates provides qualitative evidence that the GNN captured mechanistically
relevant structural features of T790M-selective inhibitors.
...
Future work should include cross-docking experiments (WT candidates into T790M
receptor and vice versa) to quantify mutant-selectivity, iterative
self-training rounds, ADMET profiling, and ultimately wet-lab validation.
